...
class gl{
	...
	//------------------------------------------------------
		...
		static fitScreen(wp = 1, hp = 1){
			gl.setSize( window.innerWidth * (wp || 1),window.innerHeight * (hp || 1) );
			return gl;
		}
	...
	//------------------------------------------------------
		static createShader(vShaderTxt, fShaderTxt, doValidate = true, transFeedbackVars = null, transFeedbackInterleaved = true){
			let vShader		= gl.compileShader(vShaderTxt, gl.ctx.VERTEX_SHADER);
			if(!vShader)	return null;

			let fShader		= gl.compileShader(fShaderTxt, gl.ctx.FRAGMENT_SHADER);
	...
	//------------------------------------------------------
		static loadTexture(name, img, doYFlip = false, useMips = false, wrapMode = 0, filterMode = 0){ 
			...
			Core.textures.set(name, tex);

			return gl.updateTexture(name,img,doYFlip,useMips);
		}

		static updateTexture(name, img, doYFlip = false, useMips = false, wrapMode = 0, filterMode = 0){ //can be used to pass video frames to gpu texture.
			let tex = Core.textures.get(name);
		...
		static loadCubeMap = function(name,imgAry){
			...
			for(let i=0; i < 6; i++){
				gl.ctx.texImage2D(gl.ctx.TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, gl.ctx.RGBA, gl.ctx.RGBA, gl.ctx.UNSIGNED_BYTE, imgAry[i]);
			}
